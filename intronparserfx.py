...
class Gene:
    ...
    def outputIntronsFromLongestTranscript(self):
        ...
        output = open('intronlist.txt', 'w')
        for i in range(len(t.exons) - 1):
            intronTupple = (self.chromosome_id, t.transcript_id, int(t.exons[i].end) + 1, int(t.exons[i + 1].start) - 1)
            intronTuppleStr = intronTupple[0] + '\t' + intronTupple[1][16:-1] + '\t' + str(intronTupple[2]) + '\t' + str(
                intronTupple[3])+'\n'
            # return intron
